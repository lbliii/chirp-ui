...
For **vertical fill** (full-height routes with inner scroll), see **`docs/fundamentals/layout-vertical.md`**.
...
Wide content is still supported: put it in a **child** with **`overflow-x: auto`** (tables, code blocks, dense toolbars). The page column clips; the child scrolls horizontally. For routes that intentionally need bounded inner vertical scroll, opt into **fill mode** — see **`docs/fundamentals/layout-vertical.md`**.
...
See also [ANTI-FOOTGUNS.md](../safety/anti-footguns.md).
