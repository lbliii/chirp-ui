...
| **HTMX-safe swap root** | `fragment_island` / `safe_region` | Wrap a widget or region that updates in place so app-shell `hx-boost` / `hx-select` does not steal the swap. See [DND-FRAGMENT-ISLAND.md](../components/dnd-fragment-island.md). |
...
- [SHELL-TABS-CONTRACT.md](../components/shell-tabs-contract.md) — sections, route tabs, and OOB handoffs
