...
> `docs/safety/verification.md`: `poe ci` is the default gate, `poe test-cov` enforces
...
> and `docs/safety/verification.md`.
...
- **2026-05-13** — Coverage/browser gate policy closed in `docs/safety/verification.md`. `poe ci` remains the trusted default gate; `poe test-cov` keeps the explicit 80% coverage floor; `poe ci-browser` is the release/browser-sensitive proof path.
