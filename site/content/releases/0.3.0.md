...
- **Provide/consume expansion** --- `surface`/`panel` provide `_surface_variant`, `card` provides `_card_variant`, `accordion` provides `_accordion_name`, `form` gains `density` param, `sidebar`/`navbar` gain `current_path`. Full key registry in `docs/components/provide-consume-keys.md`.
