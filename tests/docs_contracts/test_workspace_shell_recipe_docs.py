...
RECIPE = REPO_ROOT / "docs" / "patterns" / "workspace-shell-recipes.md"
...
def test_workspace_shell_recipe_is_linked_from_canonical_guides() -> None:
    assert "[WORKSPACE-SHELL-RECIPES.md](patterns/workspace-shell-recipes.md)" in INDEX.read_text(
        encoding="utf-8"
    )
...
def test_workspace_shell_published_bridge_points_to_durable_source() -> None:
    ...
    assert "docs/patterns/workspace-shell-recipes.md" in text
