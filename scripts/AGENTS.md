...
- **Docs site commands are explicit.** `docs-build-all` builds the Bengal site
  and emits the public manifest. Evidence:
  `pyproject.toml` poe `docs-build-all`.
...
- `scripts/docs_site.py` — site build assumptions, Bengal ownership, and public
  manifest emission.
...
related Poe tasks in `pyproject.toml`.
