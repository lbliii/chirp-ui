...
- **Strict undefined is the default (kida 0.7.0)** — missing dict keys raise `UndefinedError` instead of rendering empty. For dicts passed by callers (macro `items=` lists), use `{% if item.get("key") %}` as the guard and `{{ item.key | default("") }}` for unguarded access. Inside a `.get()` guard body, plain `{{ item.key }}` is safe because the key is proven present. `tests/test_strict_undefined.py` renders each dict-iterating component with an empty `{}` item as a regression guard — add a case when shipping a new dict-driven component.
...
- **Mobile nav drawer** — opt into `app_shell(nav_drawer=true)` (or `nav_drawer=True` in the layout render context) for a hamburger that opens the sidebar as an accessible off-canvas slide-over below 48rem (focus trap, ESC, scrim, scroll lock, focus return); with `context_rail=true` a second trigger opens the rail. **Opt-in + additive** — unset keeps the horizontal-strip fallback. The controller is **vanilla JS in `shell_runtime_script()` (no Alpine)** and repositions the *same* `<aside>` regions (no duplicated nav, so `aria-current`/OOB swaps keep working). The open drawer is `role="dialog" aria-modal="true"` and named (sidebar via static `aria-labelledby`→head title; rail via its `aria-label`). The sidebar has a server-rendered head+close; the rail has **no server head** (it's `innerHTML`-swapped by `context_rail_oob()`, so a static head would be stranded), so `shell_runtime_script()` injects a floating close into the open rail (removed on close). The hamburger lives in the always-available **non-anchor** `topbar_leading` slot (macro) / `{% block topbar_leading %}` (layout) — fill it with your own back/menu affordance, never the `brand` slot (which nests inside the brand `<a>`). Active-link state syncs via the canonical `syncNav` in `shell_runtime_script()` (mirrors per-item `match=`; match-less links are server-authoritative). Proven by `tests/browser/test_shell_nav_drawer_gauntlet.py` (in the `test-browser-chrome-check` gate). See `docs/patterns/navigation.md § Mobile Nav Drawer`.
...
- **Stable-with-composes = composition wrapper** — a `maturity="stable"` descriptor with a non-empty `composes` field is a composition wrapper, and the `maturity` field's honesty depends on it being proof-backed: it must carry the same promote-to-stable collateral as any stable promotion — either a `| Promote to stable |` row in `docs/safety/public-surface-stabilization.md` (collateral enforced by `test_promoted_public_surface_rows_have_manifest_test_and_showcase_proof`) **or** a `STABLE_COMPOSERS_WITH_PROOF` allowlist entry (in `tests/docs_contracts/test_public_surface_stabilization.py`) naming its asserting proof test. Enforced by `test_no_thin_composition_wrapper_is_stable_without_proof`. `data_table` is the canonical thin wrapper and is intentionally `experimental` (superseded by `data_grid`); complete/low-feature primitives (`table`, `calendar`, `bar_chart`, `donut`) have `composes=()` so the rule never fires on them. See `docs/safety/public-surface-stabilization.md § Stable Composition Wrappers`.
- **Server-driven data grid** — `data_grid.html` provides `data_grid` (the citable interactive-grid composite) + `grid_head_cell` (one sortable `<th>`) + `data_grid_rows` (the load-more/OOB fragment). It renders from the **typed Python helper `src/chirp_ui/grid_state.py`** (Chirp-agnostic, `route_tabs.py`-style: `Column`/`GridSort`/`ColumnSort`/`SelectionState` + `parse_sort`/`sort_columns`/`selection_state`/`column_aria_sort`/`sort_query`, exported in `__init__.__all__`, registered as template globals beside `tab_is_active`). `sort_columns()` projects each column's `aria_sort` + toggle `next_url` so the macro renders state it never computes — the server `ORDER BY` and the headers cannot drift (kills the legacy `header|lower` sort key). The sort control is a real `<button class="chirpui-table__sort">` (finally emits that orphaned hook); the single-`aria-sort` invariant is structural; focus is retained on the activated button by id after the `outerHTML` swap. Selection is the only client concern: one idempotent `chirpuiGridSelection` Alpine factory (queries via **`this.$root`**, not `$el`, since a child checkbox's `@change` rebinds `$el` to the checkbox) with page-scoped select-all; `indeterminate` is set **only** via the JS DOM property (`x-effect="$el.indeterminate = someSelected"`), never an HTML attr (it is IDL-only and would not reflect). The factory re-seeds from server-checked rows after **every** swap: the sort `outerHTML` swap re-inits it, and the load-more `beforeend` append (which keeps the `x-data` root alive, so `init()` does NOT re-fire) is re-scanned via a scoped `htmx:afterSettle` listener registered in `init()` (removed in `destroy()`) — so select-all then load-more correctly drops select-all to *indeterminate* and a server-checked appended row is re-adopted (WCAG 4.1.2; the bare load-more without this wiring lied "all selected"). Row-checkbox accessible names come from the optional `row_labels` list (clean plain text), **never** the rendered first cell (rich HTML or empty spacer), falling back to the stable row id. `table()` gained additive `selectable`/`select_name`/`selection`/`row_id`/`sticky_first_col` (legacy `sortable=`/`sort_url=` path byte-identical); `selection_bar()` gained `controlled=`. Sticky zones are pure `position: sticky` (token z-index `--chirpui-z-sticky-col` < `--chirpui-z-sticky` < `--chirpui-z-sticky-corner`) with a directional **seam shadow** (`color-mix` over `--chirpui-border`) on the pinned header bottom + pinned column inline-end edge; v1 pins the first **visual** column only (no per-column `Column(frozen=…)` — deliberately not shipped rather than advertise a no-op). The load-more `<button>` carries `hx-boost="false"` + `hx-select="unset"`/`hx-disinherit` so a boost-inherited select can't strip the bare-`<tr>` fragment. Proven by `tests/browser/test_data_grid_gauntlet.py` (in `test-browser-chrome-check`). See `docs/patterns/data-grid.md`.
...
| `{% if item.key %}` guards raise on missing keys under kida 0.7.0 `strict_undefined=True` | Converted to `{% if item.get("key") %}` + `\| default("")` across 9 dict-iterating templates (timeline, dock, segmented_control, route_tabs, nav_tree, tree_view, bar_chart, breadcrumbs, description_list, forms, config_row); added `tests/test_strict_undefined.py` as regression guard | 27 |
| Client active-sync (`syncNav`) lived only in `app_shell_layout`, used blind prefix, ignored `match=`; the macro entry point had none | Canonical `match`-mirroring `syncNav` moved into `shell_runtime_script()` (both entry points share it); `sidebar_link`/`navbar_link` emit `data-chirpui-shell-match` only when `match=` is set; match-less links are server-authoritative (JS skips them). Region announcer wrappers (`#chirpui-topbar-breadcrumbs`, `#chirpui-sidebar-nav`) added to the layout for parity | 197 |
| Interactive content in the `brand` slot/block nests inside the brand `<a>` (invalid HTML, anchor hijacks the click) | Added an always-available **non-anchor** `topbar_leading` slot (macro) / `{% block topbar_leading %}` (layout) before the brand, hosting the built-in hamburger + collapsible toggle | 220 |
| `shell_actions_bar` baked fixed overflow/menu ids that duplicate when the bar renders twice (topbar + drawer copy) | `id_suffix=""` param namespaces both the overflow id and each per-action menu id; default `""` is byte-identical for single-instance shells | 224 |
| `table`/`data_table` were thin + stable with no sort state, no `aria-sort`, no selection; fragile `header\|lower` sort key; orphaned `.chirpui-table__sort` CSS hook never emitted | New `data_grid` composite + typed `grid_state` helper (`sort_columns` projects `aria-sort` + toggle `next_url`, stable `key`); real `<button>` sort control (emits the hook); `chirpuiGridSelection` Alpine factory (page-scoped select-all, JS-property `indeterminate`); sticky first column; HTMX load-more. `data-table` demoted stable→experimental; `table`/`table-wrap` stay stable | 200 |
